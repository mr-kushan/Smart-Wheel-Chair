...
def track_neck_flexion(landmarks, state, reps, last_change_time):
    ...
    height_diff = nose_y - avg_shoulder_y  # Positive when head tilts forward
    ...
    if height_diff > threshold_down and current_time - last_change_time > cooldown:
        feedback = "Tilt head forward"
        if state == "up":
            reps += 1
            print(f"Rep {reps} completed")
        state = "down"
        last_change_time = current_time
    elif height_diff < threshold_up and current_time - last_change_time > cooldown:
...
def save_to_csv(data, filename="records/real time data/neck_flexion_data.csv"):
    df = pd.DataFrame(data, columns=["Timestamp", "Reps", "Height Diff", "Symmetry", "Feedback"])
    df.to_csv(filename, index=False)

def save_summary_to_records(exercise_name, total_reps, total_time, avg_symmetry, avg_height_diff, performance, recommended_reps, filename="records/final performance data/neck_flexion.csv"):
...
def neck_flexion_tracker(stop_event, recommended_reps=10):
    ...
    while cap.isOpened() and not stop_event.is_set():
        ...
        if results.pose_landmarks:
            ...
            reps, state, height_diff, symmetry, last_change_time, feedback = track_neck_flexion(landmarks, state, reps, last_change_time)
        ...
        cv2.imshow("Neck Flexion Tracker", frame)
    ...
    save_summary_to_records("Neck Flexion", reps, total_time, avg_symmetry, avg_height_diff, performance, recommended_reps)
