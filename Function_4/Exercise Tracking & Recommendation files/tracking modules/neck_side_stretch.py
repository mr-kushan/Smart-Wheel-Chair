...
def track_neck_side_stretch(landmarks, state, reps, last_change_time):
    ...
    if displacement > threshold_right:
        if state == "right_start" and current_time - last_change_time >= hold_time:
            reps += 1
            print(f"Rep {reps} completed")
            state = "right_complete"
            last_change_time = current_time
        elif state != "right_complete":
            state = "right_start"
            feedback = "Hold right stretch"
    elif displacement < threshold_left:
        if state == "left_start" and current_time - last_change_time >= hold_time:
            reps += 1
            print(f"Rep {reps} completed")
            state = "left_complete"
            last_change_time = current_time
        elif state != "left_complete":
            state = "left_start"
            feedback = "Hold left stretch"
    else:
        state = "neutral"

    return reps, state, displacement, symmetry, last_change_time, feedback

def save_to_csv(data, filename="records/real time data/neck_side_stretch_data.csv"):
    df = pd.DataFrame(data, columns=["Timestamp", "Reps", "Displacement", "Symmetry", "Feedback"])
    df.to_csv(filename, index=False)

def save_summary_to_records(exercise_name, total_reps, total_time, avg_symmetry, avg_displacement, performance, recommended_reps, filename="records/final performance data/neck_side_stretch.csv"):
...
def neck_side_stretch_tracker(stop_event, recommended_reps=10):
    ...
    while cap.isOpened() and not stop_event.is_set():
        ...
        if results.pose_landmarks:
            ...
            reps, state, displacement, symmetry, last_change_time, feedback = track_neck_side_stretch(landmarks, state, reps, last_change_time)
        ...
        cv2.imshow("Neck Side Stretch Tracker", frame)
    ...
    save_summary_to_records("Neck Side Stretch", reps, total_time, avg_symmetry, avg_displacement, performance, recommended_reps)
