...
def track_head_turns(landmarks, state, reps, last_change_time):
...
def save_to_csv(data, filename="records/real time data/head_turns_data.csv"):
    df = pd.DataFrame(data, columns=["Timestamp", "Reps", "Left Angle", "Right Angle", "Symmetry", "Feedback"])
    df.to_csv(filename, index=False)

def save_summary_to_records(exercise_name, total_reps, total_time, avg_symmetry, avg_rotation_angle, performance, recommended_reps, filename="records/final performance data/head_turns.csv"):
...
def head_turns_tracker(stop_event, recommended_reps=10):
    ...
    while cap.isOpened() and not stop_event.is_set():
        ...
        if results.pose_landmarks:
            ...
            reps, state, left_angle, right_angle, last_change_time, feedback = track_head_turns(landmarks, state, reps, last_change_time)
        ...
        cv2.imshow("Head Turns Tracker", frame)
    ...
    save_summary_to_records("Head Turns", reps, total_time, avg_symmetry, avg_rotation_angle, performance, recommended_reps)
